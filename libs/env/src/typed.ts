...
export const publicConfig = Effect.provide(
  Config.all({
    ...
    BACKEND_URL: Config.url('PUBLIC_BACKEND_URL').pipe(Config.withDefault(new URL('http://localhost:3001'))),
    FRONTEND_URL: Config.url('PUBLIC_FRONTEND_URL').pipe(Config.withDefault(new URL('http://localhost:3000'))),
  }),
  publicLayer,
)
