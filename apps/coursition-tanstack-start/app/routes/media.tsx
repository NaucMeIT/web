...
import {
  ...
  Tabs,
  Textarea,
} from '@nmit-coursition/ui/design-system'
...
function Media() {
  const [status, setStatus] = useState<'idle' | 'upload' | 'parse' | 'done'>(
    'idle',
  )
    ...
    try {
      ...
      const videoSource =
        parsedData.type === 'file'
          ? URL.createObjectURL(parsedData.file)
          : parsedData.url

      setStatus('parse')
      const keywordsArray = parsedData.keywords
        ? parsedData.keywords.split(',').map((word) => `${word}:5`)
        : []
      ...
      toast.error(
        `Something went wrong. Reason: ${error instanceof Error ? error.message : 'Unknown.'}`,
      )
      setStatus('idle')
      return initialState
    }
    ...
    <div className="flex justify-center h-screen">
      <div className="p-4 max-w-2xl w-full">
        ...
        {status !== 'idle' && status !== 'done' && (
          <StatusDisplay states={statusStates} status={status} />
        )}
        {status === 'done' && (
          <Tabs
