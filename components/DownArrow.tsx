import type { SVGProps } from "react"

export function DownArrow(props: Readonly<SVGProps<SVGSVGElement>>) {
    return (
        <svg
            xmlns='http://www.w3.org/2000/svg'
            width={34.453}
            height={45.507}
            viewBox='0 0 34.453 45.507'
            className='mx-auto mb-20 cursor-pointer hidden md:block'
            onClick={() => window.scrollBy(0, document.documentElement.clientHeight * 0.8)}
        >
            <g data-name='Component 14 \u2013 1' fill='none' stroke='#fff' strokeWidth={1.5}>
                <path
                    className='motion-safe:animate-pulseDown'
                    data-name='Path 4300'
                    d='M.53 16.294 17.63 33.392 33.923 17.1'
                />
                <path data-name='Path 4301' d='M4.172 31.077 17.54 44.446l12.74-12.74' />
            </g>
        </svg>
    )
}
