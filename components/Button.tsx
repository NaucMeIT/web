...
export function SocialButton({ naked, label, className, disabled, children, ...rest }: SocialButtonProps) {
	...
	const props = {
		className: `${naked ? "" : mainClasses} ${disabled ? themeClasses.disabled[theme] : themeClasses.enabled[theme]} ${
			"href" in rest ? "pointer-events-none" : ""
		} ${sizeClasses.icon} ${className ?? ""} items-center aspect-square`,
